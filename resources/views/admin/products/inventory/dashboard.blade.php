...
@section('content')
    <a href="{{ URL::secure(route('admin.product.manage')) }}">manage product</a>
    <a href="{{ URL::secure(route('admin.product.manage.stock')) }}">manage product stock</a>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function(event) {
            ...
            const options = {
                ...
                xaxis: {
                    categories: Object.keys(stockCounts)
                },
                yaxis: {
                    title: {
                        text: 'Stock'
    ...
    {{-- <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Total Variants -->
            <div class="bg-white rounded shadow-sm p-4">
                <h2 class="text-lg font-semibold mb-2">Total Variants</h2>
                <div class="text-4xl font-bold">{{ $totalVariants }}</div>
            </div>

            <div class="bg-white rounded shadow-sm p-4">
                @foreach ($variants as $variant)
                    <div>{{ $variant }}: {{ $variantCounts[$variant] }}</div>
                @endforeach
            </div>

            <!-- Stock Chart -->
            <div class="bg-white rounded shadow-sm p-4 col-span-2">
                <h2 class="text-lg font-semibold mb-2">Stock by Variant</h2>
    ...
    <!-- Load the stock data and create the chart -->
    {{-- <script>
        const stockData = @json($stockData); // Replace with actual stock data from the server
